...
int comparator(const void *a, const void *b){
    const Level *x = (Level)a;
    const Level *y = (Level)b;
    return x->level - y->level;
}
/* constructs a binary tree from ancestor matrix returns NULL, if matrix is invalid */
...
{    //Calculate level of each node (level is the num of ancestor nodes)
    ...
    qsort(levels, n, sizeof(Level),comparator);         //sort nodes based on level
    if(levels[0].level != 0)                                 //first node level should be zero
        return NULL;
    //Construct root node;
    ...
    Node *node;
    int prevLevelStart = 0,levelStart = 0,curlevel = 0;
    for(i = 1; i < n; i++){
        node = (Node *)calloc(1, sizeof(Node));         //allocate the node
